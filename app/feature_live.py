...
컬럼명/타입 스키마는 data/raw/*.csv를 직접 스캔하지 않고 배포 시점에 미리
추출해둔 models/csv_schemas.json(실데이터 0건, scripts/extract_csv_schemas.py로
생성)에서 읽는다 — 배포 환경에 data/ 자체가 필요 없도록 하기 위함(Phase 6).
...
SCHEMAS_PATH = os.path.join(os.path.dirname(__file__), "..", "models", "csv_schemas.json")
SYNTHETIC_SK_ID_CURR = -1

NUMERIC_TYPES = {"BIGINT", "DOUBLE", "FLOAT", "INTEGER", "SMALLINT", "TINYINT", "HUGEINT", "DECIMAL"}

with open(SCHEMAS_PATH) as _f:
...
def _records_to_df(csv_name: str, records: list[dict], overrides: dict) -> pd.DataFrame:
    ...
    schema = _get_csv_schema(csv_name)
    if not records:
...
def build_live_feature_row(payload: dict) -> pd.DataFrame:
    ...
    prev_df = _records_to_df(
        "previous_application", prev_records,
    ...
    ccb_df = _records_to_df("credit_card_balance", ccb_flat,
                             overrides={"SK_ID_PREV": ccb_sk_id_prev})
    pos_df = _records_to_df("POS_CASH_balance", pos_flat,
                             overrides={"SK_ID_PREV": pos_sk_id_prev})
    inst_df = _records_to_df("installments_payments", inst_flat,
